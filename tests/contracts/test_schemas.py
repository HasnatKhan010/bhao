...
def _row(week, city="05", item="019", revision=0, avg=100.0, ingested=None, **over):
    ...
    return pd.DataFrame(d).astype(
        {"revision": "int32", "qty_norm": "float64", "ingested_at": "datetime64[us, UTC]"}
    )
...
class TestAsOf:
    ...
    def test_excludes_revision_not_yet_ingested(self):
        ...
        df = pd.concat(
            [
                _row(
                    _week(1), revision=0, avg=100.0, ingested=pd.Timestamp("2026-01-10T06:00:00Z")
                ),
                _row(
                    _week(1), revision=1, avg=103.0, ingested=pd.Timestamp("2026-01-20T06:00:00Z")
                ),
            ],
            ignore_index=True,
        )
    ...
    def test_one_row_per_key(self):
        df = pd.concat(
            [
                ...
                _row(
                    _week(2), revision=0, avg=101.0, ingested=pd.Timestamp("2026-01-17T06:00:00Z")
                ),
            ],
            ignore_index=True,
        )
